    ...
    * The class will make use of an array with external linkage provided
    ...
    5) _symbols     - A pointer to the first symbol in an array with external
        ...
        * imaginary symbol  - An image depicting the imaginary number to indicate
                        ...
                        symbols.
    6) Container    - The container template used to store indices to the array.
...
#include "Precision_Int_Abstract_Base.h"
#include "Precision_Sign_Class.h"
#include "Impl/Shared_Constants.h"
#include "Impl/Precision_Tags.h"
...
namespace Precision{
    namespace General_Base{
        ...
        class Int : Tag::Integral, Tag::Signed {
            public:
        //Type aliases
                using abstract_type = Abstract::Int<
                                        ByteType, Base,
                                        Container, SignClass
                                        >;
                using str_type      = std::basic_string<CharT>;
                using lli           = typename abstract_type::lli;
                using ld            = typename abstract_type::ld;
                using image_type    = CharT;
        ...
        //Constructors and destructor
                constexpr Int(lli=0);
                Int(const str_type&);
                constexpr Int(const diglist_type&, sign_type);
                constexpr Int(const abstract_type&);
